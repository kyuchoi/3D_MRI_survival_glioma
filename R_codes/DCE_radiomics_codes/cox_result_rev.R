...
# install.packages('C:\\Users\\CNDLMembers\\Downloads\\ReporteRsjars_0.0.4.tar.gz', repos=NULL, type='source', INSTALL_opts=c("--no-multiarch"))
# install.packages('C:\\Users\\CNDLMembers\\Downloads\\ReporteRs_0.8.10.tar.gz', repos=NULL, type='source', INSTALL_opts=c("--no-multiarch"))
library(ReporteRs)
...
seed <- 81234 # 81234 # best after MGMT: 1234 /orig: 23487/ prev best: 123456 (age 0.017 and MGMT 0.005) /2nd best: 987654321 (0.00949 but IDH 0.487, age 0.049)
curv.size <- 0.5
...
if (use.preset.splitnum){
  split_num <- c(2, 3, 4, 5, 6, 8, 9, 10, 11, 13, 14, 16, 17, 19, 20, 21, 22, 25, 26, 27, 28, 30, 32, 35, 36, 38, 40, 41, 42, 43, 47, 48, 49, 50, 51, 52, 54, 55, 56, 57, 60, 61, 62, 63, 65, 66, 67, 69, 70, 71, 72, 73, 79, 80, 83, 84, 86, 87, 88, 90, 92, 93, 94, 95, 96, 97, 98, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 113, 115, 116, 117, 118, 119, 121, 122, 123, 126, 130, 131, 132, 133, 134, 135, 137, 138, 139, 140, 141, 142, 143, 144, 149) # NEVER remove 149 !!
...
if(use.train.median){
  ...
  x.test$group <- score.bin # as.factor(score.bin)
}
...
doc <- addPlot(doc, function() print(gg, newpage = FALSE), 
               vector.graphic = TRUE)
writeDoc(doc, file = file.path(result_dir, "ggforest.pptx"))
...
# x.test.surv.age <- ggcoxadjustedcurves(res.cox, data = x.test,
...
#                vector.graphic = TRUE)
writeDoc(doc, file = file.path(result_dir, "cox_each_group_test.pptx"))
