...
doc <- addPlot(doc, function() print(gg.cumhaz.train, newpage = FALSE),
               vector.graphic = TRUE)
writeDoc(doc, file = file.path(result_dir, "cox_risk_group_train_2ndrev.pptx"))
...
doc <- addPlot(doc, function() print(gg.cumhaz.test, newpage = FALSE),
               vector.graphic = TRUE)
writeDoc(doc, file = file.path(result_dir, "cox_risk_group_test_2ndrev.pptx"))

#### saving selected 5 features
write.csv(select.ft, file.path(result_dir, 'selected_features_2ndrev.csv'))
