...
```{r}
csv_path = '/Users/jolee/Dropbox/!Papers/GBM/SNUH_merged_prediction_and_label_selected (2).csv'
...
```{r}
df$MRI5_bin <- as.integer(df$MRI5 > 0.8)
...
```{r}
cox <- coxph(Surv(duration, event) ~ MRI, data=df, x = TRUE)
summary(cox)
