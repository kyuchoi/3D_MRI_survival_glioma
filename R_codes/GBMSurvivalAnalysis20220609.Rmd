...
```{r}
csv_path = '/Users/jolee/Dropbox/!Papers/GBM/SNUH_merged_prediction_and_label_selected_with_EOR.csv'
GBM_df = read.csv(csv_path)
print(nrow(GBM_df))
GBM_df %>% head()

NA_index = which(GBM_df$EOR == '')
GBM_df <- GBM_df[-c(NA_index),]
```
...
km_EOR_fit <- survfit(Surv(duration, event) ~ EOR, data=df)
km_MRI4_fit <- survfit(Surv(duration, event) ~ MRI4_bin, data=df)
km_MRI5_fit <- survfit(Surv(duration, event) ~ MRI5_bin, data=df)
...
autoplot(km_EOR_fit, conf.int=TRUE, main='EOR')
autoplot(km_MRI4_fit, conf.int=TRUE, main='MRI4')
autoplot(km_MRI5_fit, conf.int=TRUE, main='MRI5')
```
...
```{r}
df['is_total'] = df['EOR'] == 'total'
df['at_least_subtotal'] = df['EOR'] == 'total' | df['EOR'] == 'subtotal'

km_mgmt_fit <- survfit(Surv(duration, event) ~ is_total, data=df)
km_EOR_fit <- survfit(Surv(duration, event) ~ at_least_subtotal, data=df)

autoplot(km_mgmt_fit, conf.int=TRUE, main='is_total')
autoplot(km_EOR_fit, conf.int=TRUE, main='at_least_subtotal')
```
...
```
```{r}
cox <- coxph(Surv(duration, event) ~ IDH + MGMT + MRI4_bin, data=df, x = TRUE)
summary(cox)
```
```{r}
BIC(cox)
...
combinations = expand.grid(c('1', 'at_least_subtotal'), c('1','is_total'),c('1', 'age'),c('1', 'IDH'),c('1', 'MGMT'),c('1', 'EOR'),c('1','MRI4'))
