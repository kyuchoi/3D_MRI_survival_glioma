...
def surv_likelihood(n_intervals):
  ...
  def loss(y_true, y_pred):
    ...
    return torch.sum(-torch.log(torch.clip(torch.concatenate((cens_uncens,uncens)),torch.epsilon(),None)),axis=-1) #return -log likelihood
  return loss
...
class PropHazards(Layer):
    ...
    def call(self, x):
        ...
        return torch.pow(torch.sigmoid(self.kernel), torch.exp(x))
